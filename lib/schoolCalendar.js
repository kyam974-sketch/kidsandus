...
export const NURSERY_SESSIONS_PER_STORY = 8;
export const NURSERY_STORIES = 4;

// Official 2026/27 part windows reconstructed from MyClassroom planning.
// Regular in-school classes follow these windows. Nursery courses instead use
// their own start date and advance every 8 actual lessons.
...
export function isNurseryCourse(course) {
  return /\bnursery\b/i.test(String(course || ''));
}
...
function firstWeekdayOnOrAfter(value, weekday) {
  const start = parseDate(value || COURSE_START);
  if (!start) return null;
  const target = Number(weekday);
  const current = new Date(start);
  const delta = (target - isoWeekday(current) + 7) % 7;
  current.setDate(current.getDate() + delta);
  return current;
}

export function nurseryPlanFromStart(weekday, startDate) {
  const first = firstWeekdayOnOrAfter(startDate, weekday);
  const end = parseDate(COURSE_END);
  if (!first || !end) return [];
  const dates = [];
  const current = new Date(first);
  const maxSessions = NURSERY_SESSIONS_PER_STORY * NURSERY_STORIES;

  while (current <= end && dates.length < maxSessions) {
    if (!isSchoolClosure(current)) dates.push(isoDate(current));
...
export function scheduledLessonForDate(item, value) {
  ...
  const nurseryRelative = item?.schedule_mode === 'relative' || isNurseryCourse(item?.course);

  if (nurseryRelative && isNurseryCourse(item?.course)) {
    const classStart = item?.start_date || officialCourseStartForWeekday(item?.weekday);
    const dates = nurseryPlanFromStart(item.weekday, classStart);
    const index = dates.indexOf(currentIso);
    if (index < 0) return null;
    return {
      story: Math.floor(index / NURSERY_SESSIONS_PER_STORY) + 1,
      day: (index % NURSERY_SESSIONS_PER_STORY) + 1,
      sessionIndex: index,
      totalSessions: NURSERY_SESSIONS_PER_STORY,
      date: currentIso,
    };
  }
